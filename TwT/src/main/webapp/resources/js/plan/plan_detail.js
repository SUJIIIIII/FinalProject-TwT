...
$(document).ready(function(){
   ...
   $(document).on("click", "#cat_menu li", function() {
     ...
     paths = [];   
      ...
      var city_lat = $(".day_menu.on").data("lat");
      var city_lng = $(".day_menu.on").data("lng");
       ...
       $.ajax({
          ...
          success : function(data) {
             ...
             if($("schedule_detail_box").children().length == 0) {
              ...
              } else if($("#schedule_detail_box").children().length>0){
...
function insertPlan(){         
   ...
   var send_file = $("#form")[0];
   var form_file = new FormData(send_file);
   ...
   //total_obj = {'form_data' : form_data, 'day_list' : day_list};
   total_obj['form_data'] = form_arr;
   total_obj['day_list'] = day_list; 
   total_obj['form_file'] = form_file;
